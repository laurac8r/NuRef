...
// we want to get the kinetic energy of the current event 
  ...
  //G4cout<<"E is: "<<ke<<G4endl;
   // bins neutrons enterin the volume
  if (pre->GetStepStatus() == fGeomBoundary && name == "Target_logical" && particle =="neutron")
  {
    G4double kinetic_energy = aStep->GetTrack()->GetKineticEnergy();
    analysisManager->FillH1(0,kinetic_energy);
//bins neutrons that has suffered scattering
    if(kinetic_energy < ke){analysisManager->FillH1(1,kinetic_energy);}
