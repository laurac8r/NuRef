...
void SteppingAction::UserSteppingAction(const G4Step* aStep)
{
  ...
  G4TouchableHandle touch_volume = post->GetTouchableHandle()
  G4String name = post->GetPhysicalVolume()->GetLogicalVolume()->GetName();
  G4double charge = aStep->GetTrack()->GetDefinition()->GetPDGCharge();
  G4String particle = aStep->GetTrack()->GetDefinition()->GetParticleName(); 
   // counts gamma leaving the target and entering the tallying volume surrounding the target, and store their energy
  if (post->GetStepStatus() == fGeomBoundary && name == "Target_logical" && particle =="neutron")
  {
    G4double kinetic_energy = aStep->GetTrack()->GetKineticEnergy();
    G4double energy_deposited = aStep->GetTotalEnergyDeposit();
    analysisManager->FillH1(1,kinetic_energy);
    // fEventAction->CountStepsCharg(); //re-engineered to count gamma photons
    analysisManager->FillH1(3,energy_deposited);// electron energy exiting the target
